...
public:
    ...
    DataHandler(tf::TransformListener& tflIn, double visibilityTimeout, double intWindowIn, const Eigen::MatrixXd& muIn, const Eigen::MatrixXd& covIn) : tfl(tflIn)
    {
        ...
        {
            estimatorOn = true;
            switchingTimer = nh.createTimer(ros::Duration(60.0),&DataHandler::switchingTimerCB,this,true);
        }
        else
        {
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        Eigen::Quaterniond quat(tfRelPose.getRotation().getW(),tfRelPose.getRotation().getX(),tfRelPose.getRotation().getY(),tfRelPose.getRotation().getZ());
        quat.normalize();
        ...
        outMsg.vc[0] = vCc(0);                   outMsg.vc[0] = vCc(1);                   outMsg.vc[2] = vCc(2);
        outMsg.wc[0] = wGCc(0);                  outMsg.wc[1] = wGCc(1);                  outMsg.wc[2] = wGCc(2);
        outMsg.vt[0] = vTt(0);                   outMsg.vt[0] = vTt(1);                   outMsg.vt[2] = vTt(2);
